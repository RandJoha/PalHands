...
async function debugDanaProvider() {
  try {
    // Connect to MongoDB
    ...
    // Check if any provider has null/undefined providerId
    const nullProviderIds = danaProviders.filter(p => p.providerId == null);
    if (nullProviderIds.length > 0) {
      console.log('\n⚠️  WARNING: Found providers with null/undefined providerId!');
      nullProviderIds.forEach(provider => {
