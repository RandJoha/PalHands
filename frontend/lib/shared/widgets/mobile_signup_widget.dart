...
class _MobileSignupWidgetState extends State<MobileSignupWidget> {
  ...
  void initState() {
    super.initState();
    // Pre-select user type if provided in arguments
    if (widget.arguments != null && widget.arguments!['userType'] != null) {
      _selectedUserType = widget.arguments!['userType'];
    }
  }
  ...
  int _getMaxSteps() {
    ...
    if (_selectedUserType == 'provider') return 1;
    return 0;
  }
  ...
  // Create services based on selected categories during signup
  Future<void> _createServicesForProvider() async {
    if (_selectedUserType != 'provider' || _selectedMainCategory == null) {
      return;
    }

    try {
      // Show loading message for service creation
      if (mounted) {
      ...
      // Get category name
      final categoryName = _getCategoryName(_selectedMainCategory!, languageService);
      
      // Create services based on the selected category and subcategories
      List<bool> results = [];
      
      // Create main category service
      if (_selectedMainCategory != null) {
        final mainServiceSuccess = await providerService.createService(
          title: categoryName,
          description: 'Professional ${categoryName.toLowerCase()} service',
          category: _selectedMainCategory!,
          subcategory: _selectedSubCategory ?? '',
          price: {
            'amount': 25.0, // Default price
            'type': 'hourly',
            'currency': 'ILS',
          },
        );
        results.add(mainServiceSuccess);
      }
      
      // Create additional services for subcategories if selected
      if (_selectedSubCategory != null && _selectedSubCategory!.isNotEmpty) {
        final subCategoryName = AppStrings.getString(_selectedSubCategory!, languageService.currentLanguage);
        final subServiceSuccess = await providerService.createService(
          title: subCategoryName,
          description: 'Specialized ${subCategoryName.toLowerCase()} service',
          category: _selectedMainCategory!,
          subcategory: _selectedSubCategory!,
          price: {
            'amount': 30.0, // Slightly higher price for specialized service
            'type': 'hourly',
            'currency': 'ILS',
          },
        );
        results.add(subServiceSuccess);
      }

      // Show success message
      if (results.isNotEmpty && results.every((result) => result)) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Services created successfully based on your selection!',
                style: GoogleFonts.cairo(color: AppColors.white),
              ),
      ...
      } else if (results.isNotEmpty && results.any((result) => result)) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Some services were created successfully.',
                style: GoogleFonts.cairo(color: AppColors.white),
              ),
