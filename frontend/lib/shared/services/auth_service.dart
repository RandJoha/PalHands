...
class AuthService extends ChangeNotifier with BaseApiService {
  ...
  Future<void> initialize() async {
    try {
      ...
      // Clear any invalid stored data (like rand@palhands.com or any corrupted data)
      if (savedUser != null || savedToken != null) {
        try {
          if (savedUser != null) {
            final userData = json.decode(savedUser);
            if (userData is Map<String, dynamic> && 
                (userData['email'] == 'rand@palhands.com' || 
                 userData['email'] == 'abeer@palhands.com' ||
                 userData['email']?.toString().contains('rand') == true)) {
              if (kDebugMode) {
                print('🧹 Clearing invalid stored credentials: ${userData['email']}');
              }
          ...
          // If JSON parsing fails or any error occurs, clear all data
          if (kDebugMode) {
            print('🧹 Clearing corrupted stored data due to error: $e');
          }
          await _clearPersistedData();
          return;
        }
